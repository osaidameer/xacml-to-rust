...
fn evaluate_cond_policy4_rule2(inp: &Inputs) -> bool {
    (inp.access_subject_age - inp.environment_bart_simpson_age) >= 5
}

fn evaluate_rule_policy4_rule2(inp: &Inputs) -> Result {
    if evaluate_cond_policy4_rule2(inp) {
        return Result::Permit;
    } else {
        return Result::NotApplicable;
    }
...
fn evaluate_policy_policy4(inp: &Inputs) -> Result {
    ...
    let results = vec![evaluate_rule_policy4_rule2(inp)];
