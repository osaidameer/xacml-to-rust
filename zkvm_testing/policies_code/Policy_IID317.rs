...
fn evaluate_cond_policy5_rule5(inp: &Inputs) -> bool {
    (inp.access_subject_age - inp.environment_bart_simpson_age) >= 5
}

fn evaluate_rule_policy5_rule5(inp: &Inputs) -> Result {
    if evaluate_cond_policy5_rule5(inp) {
...
fn evaluate_cond_policy3_rule3(inp: &Inputs) -> bool {
    inp.access_subject_bogus == "Zaphod Beedlebrox"
}
...
fn evaluate_target_policy5(inp: &Inputs) -> bool {
    true
}

fn evaluate_policy_policy5(inp: &Inputs) -> Result {
    if !evaluate_target_policy5(inp) {
        return Result::NotApplicable;
    }

    let results = vec![evaluate_rule_policy5_rule5(inp)];
